...
class AbstractRescueGridEnv:
    ...
    def __init__(
        ...
        reward_step_penalty: float = 0.0,  # 기본 step 패널티
        reward_collision: float = -0.05,  # 충돌 패널티
        reward_closer: float = 0.10,  # 가까워질 때 보상
        reward_farther: float = -0.03,  # 멀어질 때 패널티
        reward_found_victim: float = 3.0,  # 피해자 발견 보상
        use_random_heading_on_reset: bool = True,  # reset 시 방향 랜덤 여부
        n_victims: int = 3,  # 피해자 수
    ) -> None:
        ...
        self.n_victims = int(n_victims)  # 피해자 수 저장
        ...
        self.victim_positions: Set[Tuple[int, int]] = set()  # 남아 있는 피해자 위치들
        self.step_count: int = 0  # 현재 step 수
        self.done: bool = False  # 종료 여부

        self.last_distance_to_victim: float = 0.0  # 마지막 최근접 피해자 거리
        self.episode_index: int = 0  # 에피소드 번호
        self.rescued_count: int = 0  # 구조한 피해자 수
        self.initial_victim_count: int = self.n_victims  # 초기 피해자 수
    ...
    def reset(self) -> Dict[str, float]:
        ...
        # Pick free cells for agent and victims.
        ...
        for vr, vc in self.victim_positions:
            self.grid[vr, vc] = 0  # 피해자 칸 보정

        self.agent_heading = int(self.rng.integers(0, 4)) if self.use_random_heading_on_reset else 0  # 초기 방향 설정
        self.last_distance_to_victim = self._nearest_victim_distance(self.agent_pos)  # 초기 최근접 거리 저장
    ...
    def step(self, action: Action) -> EnvStepResult:
        ...
        old_distance = self._nearest_victim_distance(old_pos)  # 이전 최근접 거리
        ...
        new_distance = self._nearest_victim_distance(self.agent_pos)  # 새 최근접 거리
        found_victim = self.agent_pos in self.victim_positions  # 피해자 발견 여부
        ...
        if found_victim:
            self.victim_positions.remove(self.agent_pos)  # 해당 피해자 제거
            self.rescued_count += 1  # 구조 수 증가
            reward += self.reward_found_victim  # 피해자를 찾았으면 큰 보상 추가

            if len(self.victim_positions) == 0:  # 모든 피해자 구조 완료
                self.done = True

        if self.step_count >= self.max_steps:  # 최대 step 도달하면 종료
            self.done = True

        self.last_distance_to_victim = self._nearest_victim_distance(self.agent_pos)  # 마지막 최근접 거리 저장
        ...
        info = {
            ...
            "moved": moved,  # 이동 여부
            "distance_to_nearest_victim": float(new_distance),  # 현재 최근접 거리
            "found_victim": found_victim,  # 발견 여부
            "remaining_victims": int(len(self.victim_positions)),  # 남은 피해자 수
            "rescued_count": int(self.rescued_count),  # 구조한 피해자 수
    ...
    def get_observation(self) -> Dict[str, float]:
        ...
        victim_signal = self._victim_signal_strength()  # 최근접 피해자 신호 세기
    ...
    def render_ascii(self) -> str:
        ...
        for r in range(self.height):  # 각 행 순회
            ...
            for c in range(self.width):  # 각 열 순회
                ...
                elif pos in self.victim_positions:  # 남아 있는 피해자 위치면
    ...
    def get_env_state(self) -> Dict[str, Any]:
        return {
            ...
            "victim_positions": sorted(list(self.victim_positions)),  # 남아 있는 피해자 위치들
            "remaining_victims": len(self.victim_positions),  # 남은 피해자 수
            "rescued_count": self.rescued_count,  # 구조한 피해자 수
    ...
    def _nearest_victim_distance(self, pos: Tuple[int, int]) -> float:
    ...
    def _victim_signal_strength(self) -> float:
        if not self.victim_positions:
            return 0.0
        d = self._nearest_victim_distance(self.agent_pos)  # 최근접 피해자까지 거리
...
if __name__ == "__main__":
    env = AbstractRescueGridEnv(seed=42, n_victims=3)  # 환경 생성
    obs = env.reset()  # 초기화
