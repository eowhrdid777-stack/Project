...
class MemristiveSNNNetwork:
    ...
    def __init__(
        ...
        if hidden_layer is None:
            hidden_layer = MemristiveLIFOutputLayer(crossbar=input_hidden_crossbar, seed=self.seed + 201)
    ...
    def _hidden_currents(self, input_spikes: Sequence[float]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        rec = self._validate_vector(self.prev_hidden_spikes, self.hidden_dim, "prev_hidden_spikes")
    ...
    def decide(self, observation: ObsType) -> NetworkDecision:
        ...
        for local_t, enc_out in enumerate(window):
            ...
            if selected_action < 0 and output_out.winner >= 0:
                ...
                selected_hidden_pre = input_spikes.copy()
        ...
        if selected_action < 0:
            if not self.force_action_on_no_spike:
                raise RuntimeError("No output spike winner found and fallback is disabled.")

            # Debug score only: what the hidden neuron would receive from the
            # accumulated input path without recurrent feedback.
